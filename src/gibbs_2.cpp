...
// draw a value from a gamma distribution ###################test e-5###############
...
inline void copy_cube_to_4Darray(const arma::cube &src, const int l, Rcpp::NumericVector &dest) {
...
Rcpp::NumericVector cube_to_rcpp(const arma::cube &c, const std::vector<std::string> &names) {
...
Rcpp::List GibbsSamplerCpp(
	...
	for(int k = 0; k < burn+eval; k++) {
		...
		if(k >= burn) {
			...
			double logPriorP = arma::accu( (Ap+1)%log(Bp) - mat_lgamma(Ap+1) + Ap%log(P) -(Bp)%P );
			//LogPriorE=log(P(Ehat|Thetahat))
			double logPriorE = arma::accu( (Ae+1)%log(Be) - mat_lgamma(Ae+1) + Ae%log(E) -(Be)%E );
			//LogPriorAp=log(P(Aphat|Omega))
			double logPriorAp= i*n*log(lp) -lp*arma::accu(Ap);
			//LogPriorBp=log(P(Bphat|Omega))
			double logPriorBp= i*n*(ap*log(bp) -lgamma(ap))  + (ap-1)*arma::accu(log(Bp)) -bp*arma::accu(Bp);
			//LogPriorAe=log(P(Aehat|Omega))
			double logPriorAe= n*j*log(le) -le*arma::accu(Ae);
			//LogPriorBe=log(P(Behat|Omega))
			double logPriorBe= n*j*(ae*log(be) -lgamma(ae))  + (ae-1)*arma::accu(log(Be)) -be*arma::accu(Be);
			//Likelihood numerator
			double mlhood_num = logPosteriorZ+logPriorP+logPriorE+logPriorAp+logPriorBp+logPriorAe+logPriorBe;
		...
		double logsum=0;
		const arma::uword dimsl[3] = {(arma::uword)i, (arma::uword)j, (arma::uword)n};
