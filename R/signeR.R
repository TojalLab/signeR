signeR<-function(M, Mheader=TRUE, samples = "rows",
  ...
  #
  # Input data
  # M: matrix of mutation counts, each row corresponds to one sample (dimension = nsamples x 96). Can be a path to a file.
  # Mheader: if M is to be read from a file, does it contains a header line? 
  #          Otherwise, if M is a matrix, does it have mutations and samples as colnames and rownames?  
  # samples: if "rows" M is supposed to contain one sample per row. Otherwise, one sample per column is expected.
  # Opport: matrix of mutation opportunities, each column corresponds to one sample (96 x nsamples)
  # Opportheader: if Opport is to be read from a file, does it contains a header line?
  #
  ...
  # varp.ap: variance of the gamma distribution used to generate proposals for shape parameters of signatures
  # varp.ae: variance of the gamma distribution used to generate proposals for shape parameters of exposures
  #
  # Sampler parameters
  # testing_burn: number of burning iterations of the Gibbs sampler used to estimate the number of signatures in data. 
  #               Also used as the number of burning iterations on any run of the sampler with fixed variables.
  # testing_eval: number of iterations of the Gibbs sampler used to estimate the number of signatures in data. 
  #               Also used as the number of iterations on any run of the sampler with fixed variables.
  ...
  if(is.character(Opport)){ 
    ...
    Opportread<-M
    Opportread[,]<-1
  }else stop("Opport should be a matrix, a data frame, a link to a file or NA.")
